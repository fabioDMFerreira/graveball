...
rays.s=[	new THREE.Vector3(0,1,0),
	...
	var Plate=function(Z,Y,X,atrito,texture){
		...
		var obj=new THREE.Mesh(new THREE.CubeGeometry(z, y, x),new THREE.MeshLambertMaterial({map:THREE.ImageUtils.loadTexture(texture)}));
	...
	var coisasFisica=function(obj,Radius,maxVel,rot){
		...
		obj.detectCollisionUp=function(){
			...
			var auxIntersect=new Array();//onde vai ficar informacao da colisao
			auxIntersect[0]={};
			...
			for(var id in rays.up){
				obj.caster.set(obj.position,rays.up[id]);
				var intersect=obj.caster.intersectObjects(platform);
		...
		obj.collision=function(tecla,eixo){
			...
			for(var id in rays[tecla]){
					...
					if(intersect.length>0 && intersect[0].distance < obj.Radius+obj.Velocity[eixo]+15){
						...
						$(".pontuacao").html(objCatchable.length);
					}
			}
			//detectar objectos que se podem mover
			for(var id in rays[tecla]){
					obj.caster.set(obj.position,rays[tecla][id]); //atribui ponto de referencia e direcçao
					var intersect=obj.caster.intersectObjects(objMovable);//procura numa lista de objectos os que intersecta
	...
	var BallObj=function(radius){
		//construtor
		var material= new THREE.MeshLambertMaterial({map:THREE.ImageUtils.loadTexture('imagens/ball.jpg')});
	...
	var CubeObj=function(x,y,z){
		//construtor
		var obj=new THREE.Mesh(new THREE.CubeGeometry(x,y,z), new THREE.MeshLambertMaterial({map:THREE.ImageUtils.loadTexture('imagens/box.jpg')}));
		obj.castShadow=true;
		obj.structure="cube";
	...
	function init(){
		...
		ball.position=new THREE.Vector3(0,500,0);
		scene.add(ball);
		//platform.push(ball);
		//objMovable.push(ball);
		world.setObject(ball);

		var plane =new Plate(10000,10,10000,0,'imagens/bright_squares256.png');
      	plane.position=new THREE.Vector3(0,0,0);
      	plane.receiveShadow=true;
      	platform.push(plane);
		scene.add(plane);

		var wall =new Plate(100,1000,10000,0.05,'imagens/brick.JPG');
      	wall.position=new THREE.Vector3(5000,0,0);
      	wall.receiveShadow=true;
      	platform.push(wall);
		scene.add(wall);

		var wall2 =new Plate(10000,1000,100,0.05,'imagens/brick.JPG');
      	wall2.position=new THREE.Vector3(0,0,5000);
      	wall2.receiveShadow=true;
      	platform.push(wall2);
		scene.add(wall2);

		var tower =new Plate(500,500,500,0.05,'imagens/street.jpg');
      	tower.position=new THREE.Vector3(0,150,0);
      	tower.receiveShadow=true;
      	platform.push(tower);
		scene.add(tower);

		var r1 =new Plate(500,100,1500,0.05,'imagens/street.jpg');
		r1.rotation.x=-1/9*Math.PI;
      	r1.position=new THREE.Vector3(0,50,-900);
      	r1.receiveShadow=true;
      	platform.push(r1);
		scene.add(r1);

		var r2 =new Plate(500,100,1500,0.05,'imagens/street.jpg');
		r2.rotation.x=1/9*Math.PI;
      	r2.position=new THREE.Vector3(0,50,900);
      	r2.receiveShadow=true;
      	platform.push(r2);
		scene.add(r2);

		var r3 =new Plate(1500,100,500,0.05,'imagens/street.jpg');
		r3.rotation.z=1/9*Math.PI;
      	r3.position=new THREE.Vector3(-900,50,0);
      	r3.receiveShadow=true;
      	platform.push(r3);
		scene.add(r3);

		var r4 =new Plate(1500,100,500,0.05,'imagens/street.jpg');
		r4.rotation.z=-1/9*Math.PI;
      	r4.position=new THREE.Vector4(900,50,0);
      	r4.receiveShadow=true;
      	platform.push(r4);
		scene.add(r4);

		var tower2 =new Plate(2000,800,3000,0.05,'imagens/street.jpg');
      	tower2.position=new THREE.Vector3(-2000,0,2000);
		...
		cube.position=new THREE.Vector3(700,1500,700);
		scene.add(cube);
		platform.push(cube);
